...
//		request.setAttribute("devicesId", request.getParameter("devicesId"));
	...
	public String test(){
		DictUsers user = (DictUsers) request.getSession().getAttribute("session_user");
		int totalRows=deviceService.getDevicesInfoCount(deviceStatus, 
				teamId, receiverName, "3090", user.getAreaId());
		System.out.println("totalRows="+totalRows);
		Pager pager=PagerHelper.getPager(currentPage+"", "next", totalRows);
		System.out.println("pager="+pager);
		currentPage=pager.getCurrentPage();
		System.out.println("pager123="+currentPage);
		
		List list=deviceService.findDevicesInfoPage(pager.getPageSize(), pager.getStartRow(), deviceStatus, 
				teamId, receiverName, "3090", user.getAreaId());
