...
public class RolesDaoImpl extends AbstractDao implements RolesDao {
	...
	public void deleteRoleFunPrivsOfSQL(long roleID) {
		String sql = "delete from ROLES_FUNCTION r where r.ROLE_ID=?";
		getSession().createSQLQuery(sql).setLong(0, roleID).executeUpdate();
		
	}
	
	public List<RolesFunction> getRolesFunctionByIdOfSQL(long roleID){
		String sql = "select * from ROLES_FUNCTION r where r.ROLE_ID=?";
		return getSession().createSQLQuery(sql).addEntity(RolesFunction.class).setLong(0, roleID).list();
	}
	
	@SuppressWarnings("unchecked")
	public List<Object> getFuncIdByIdOfSQL(long roleID) {
		String sql = "select FUNC_ID from ROLES_FUNCTION r where r.ROLE_ID=?";
		return getSession().createSQLQuery(sql).setLong(0, roleID).list();
	}


	public List<Object> listSecFuncIdOfFunctionPrivs() {
		String sql = "select FUNC_ID from DICT_FUNCTIONS r where r.PARENTID is not null";
		return getSession().createSQLQuery(sql).list();
	}


	public Object getFuncNameById(long funcId) {
		String sql = "select FUNC_NAME from DICT_FUNCTIONS r where r.FUNC_ID=?";
		return getSession().createSQLQuery(sql).setLong(0, funcId).uniqueResult();
	}
}
