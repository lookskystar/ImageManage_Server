...
public class UsersDaoImpl extends AbstractDao implements
	...
	public PageModel<DictUsers> findDictUsers(Long areaId,String name,String gonghao,Long teamId) {
	...
	public List<DictUsers> findDictUsersPage(int pageSize, int startRow,
		...
		builder.append("from DictUsers t where t.isuser=1");
		if(areaId!=null){
			builder.append("  and t.areaId = "+areaId);
		}
		if(name!=null&&!name.equals("")){
			builder.append("  and t.name like '%"+name+"%'");
		}
		if(gonghao!=null&&!gonghao.equals("")){
			builder.append("  and t.gonghao like '%"+gonghao+"%'");
		}
		if(teamId!=null){
			builder.append("  and t.teamId = "+teamId);
		}
		builder.append(" order by t.userId");
	...
	public int getDictUsersCount(Long areaId, String name, String gonghao,
			Long teamId) {
		StringBuilder builder=new StringBuilder();
		builder.append("select count(t.userId) from DictUsers t where t.isuser=1");
		if(areaId!=null){
			builder.append("  and t.areaId =  "+areaId);
		}
		if(name!=null&&!name.equals("")){
